...
function SafePathLabyrinth() {
  ...
  const [minMaxRadiation, setMinMaxRadiation] = useState(null);
  ...
  const updateRadiationLevel = (rowIndex, colIndex, value) => {
  ...
  const findSafestPath = () => {
    ...
    setTimeout(() => {
      ...
      const distances = Array(N).fill().map(() => Array(M).fill(Infinity));
      ...
      const queue = [{ row: 0, col: 0, maxRad: radiationLevels[0][0] }];
      
      // Track paths
      const prev = Array(N).fill().map(() => Array(M).fill(null));
